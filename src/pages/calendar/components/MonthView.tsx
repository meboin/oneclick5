...
export default function MonthView({ events, selectedTemplate, onAddEvent, onDeleteEvent, onEditTemplate }: MonthViewProps) {
  ...
  return (
    <div className="flex-1 bg-white" onClick={handleClickOutside}>
      {/* 월간 헤더 */}
      {/*
        주간 보기와 크기 비율을 맞추기 위해 헤더의 폰트와 버튼 크기를 줄였습니다.
        제목은 text-lg, 오늘 버튼은 text-xs로 설정하여 전체적으로 축소된 느낌을 줍니다.
      */}
      <div className="flex items-center justify-between p-4 border-b border-gray-200">
        <div className="flex items-center space-x-3">
          ...
          <button
            onClick={goToToday}
            className="px-2 py-0.5 text-xs bg-blue-100 text-blue-700 rounded-md hover:bg-blue-200 transition-colors cursor-pointer whitespace-nowrap"
          >
            오늘
          </button>
        </div>
        <div className="flex items-center space-x-1">
          <button
            ...
            className="w-6 h-6 flex items-center justify-center text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-md transition-colors cursor-pointer"
          >
      ...
      {/* 요일 헤더 */}
      {/* 패딩과 글자 크기를 줄여 주간 보기와 유사한 비율로 조정 */}
      <div className="grid grid-cols-7 border-b border-gray-200">
        {DAYS.map(day => (
          <div
            key={day}
            className="p-2 text-center font-medium text-gray-900 border-r border-gray-200 last:border-r-0 text-xs"
          >
            {day}
          </div>
        ))}
      ...
      {/* 달력 그리드 */}
      <div className="grid grid-cols-7" style={{ height: 'calc(100vh - 300px)' }}>
        {calendarDays.map((dayData, index) => {
          ...
          return (
            <div
              key={index}
              className={`border-r border-b border-gray-200 last:border-r-0 p-2 cursor-pointer transition-colors min-h-[100px] ${bgClass}`}
                ...
                <div
                  className={`text-xs font-medium mb-1 ${
                    !dayData.isCurrentMonth
                      ? 'text-gray-400'
                      : isTodayCell
                      ? 'text-blue-600 bg-blue-100 w-5 h-5 rounded-full flex items-center justify-center'
                      : 'text-gray-900'
                  }`}
                >
                  {dayData.date}
                ...
                >
                  {dayEvents.length > 0 && dayData.isCurrentMonth && (
                    <div className="text-center cursor-pointer hover:scale-110 transition-transform">
                      <div className="w-6 h-6 bg-blue-500 text-white rounded-full flex items-center justify-center text-[9px] font-medium mb-0.5 hover:bg-blue-600">
                        {dayEvents.length}
                      </div>
                      <div className="text-[8px] text-gray-600">일정</div>
                    </div>
                  )}
                </div>
              </div>
      ...
      {contextMenu && (
        <div
          className="fixed bg-white border border-gray-200 rounded-lg shadow-lg py-1.5 z-50"
        ...
        >
          ...
          >
            <i className="ri-edit-line w-3 h-3 flex items-center justify-center mr-1"></i>
            수정
          </button>
          <button
          ...
          >
            <i className="ri-delete-bin-line w-3 h-3 flex items-center justify-center mr-1"></i>
            삭제
          </button>
        </div>
      )}
